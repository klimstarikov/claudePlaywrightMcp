...
description: Use when a test scenario needs manual execution, selector discovery, or defect investigation before automation — from a feature file, plain text description, or optionally a TMS case. Produces an Automation-Friendly Spec (AFS); does not write test code.
license: Apache-2.0
metadata:
  authors:
...
Execute a test scenario against the live app, observe what actually
happens, and emit an **Automation-Friendly Spec (AFS)** a downstream
engineer can implement without re-exploring. The scenario may come
from a Gherkin feature file, a plain text description, or optionally
a TMS case (when a TMS adapter is configured).

**Core philosophy:** a written scenario is a hypothesis. The app is
the only source of truth. This skill never trusts the scenario as
...
**Role.** Execute one test scenario end-to-end against the live app,
...
- Scenario source — one of:
  - Gherkin feature file path (e.g. `features/sale-tags.feature`)
  - Plain text scenario description
  - TMS case ID (e.g. `SCRUM-T101`) — only when a TMS adapter is configured in `.agents/test-automation.yaml`
...
- **Status** — one of `ready-for-automation` / `already-covered` /
  ...
  Phase 0 (out-of-scope, TMS only) and § 5 Classify findings (the rest).
- **AFS path** — `test-specs/<feature>/l<pri>_<slug>.md` for
  fresh-implementation, `lcovered_*` for already-covered, `lextend_*`
  for extend-existing. Omit TMS ID from slug unless one was provided.
  Omitted for `un-automatable` and `out-of-scope-by-author` (no AFS
  emitted).
- **Filed bug IDs** — if `defect-found`, the tracker IDs created
  per § 5's bug-filing routing.

## Phase 0 — Case-gate (TMS only, optional)

**Skip Phase 0 entirely when the scenario source is a feature file or plain text** — proceed directly to Phase 1.

Phase 0 applies only when a TMS case ID is provided AND a TMS adapter is configured in `.agents/test-automation.yaml`. In that case, probe the case's author metadata before fetching the body:

- **Status excluded** (`Out of Scope`, `Draft`, `Deprecated`) → return `out-of-scope-by-author` with the field value as evidence.
- **Folder/membership mismatch** → return to the orchestrator with the discrepancy.
- **TMS unreachable** → fall back to fetching the body in Phase 1; flag the gap for scout.
- **All clear** → continue to Phase 1.

## The six-phase loop (one case at a time, runs AFTER Phase 0)

```
1. Fetch the scenario     → feature file / plain text / TMS adapter (see Phase 1)
...
6. Emit AFS               → test-specs/<feature>/l<pri>_<slug>.md
```

### 1. Fetch the scenario

Choose the source in this priority order:

1. **Gherkin feature file path provided** → read the `.feature` file directly. Extract the relevant Scenario or Scenario Outline block.
2. **Plain text provided** → use as-is. Treat each sentence / bullet as a step.
3. **TMS case ID provided AND adapter configured** in `.agents/test-automation.yaml` → use the adapter (MCP if `transport: mcp`, otherwise HTTP with `auth_env`). If the TMS is unreachable, open the case in the browser and copy it by hand — do not block on a flaky TMS.
4. **No source provided** → read from `features/` and ask the caller which scenario to execute.

Extract: name, priority, preconditions, steps, expected result, cleanup (if any).
...
- **already-covered** — Rule-6 behavioural-equivalence dedup against
  an existing merged spec. The observable this scenario asserts is
  already proven by another spec on file. No own implementation
  needed. Emit a *traceability AFS* at
  `test-specs/<feature>/lcovered_<slug>.md` containing the
  **dedup proof**: covering spec at `file:line` + a one-paragraph
  behavioural-equivalence argument (why the existing assertion
  satisfies this scenario's expected observable). The `lcovered_`
  filename prefix is the contract — downstream audits grep for it to
  enumerate Rule-6-dedup coverage distinct from fresh-implementation
  coverage.
- **extend-existing** — Rule-6 *partial*-overlap. An existing merged
  spec covers most of this scenario's observable, but a small number
  of assertions are missing. Don't write a fresh `.spec.ts`; the
  implementer extends the covering spec with the gap assertions.
  Emit an *extension AFS* at
  `test-specs/<feature>/lextend_<slug>.md` containing: the covering
  spec at `file:line`, a one-paragraph behavioural-overlap argument
  (what's already proven), and a **Gap assertions** section listing
  exactly what the existing spec doesn't cover (the new selectors /
  observations / expecteds the implementer needs to append). The
  `lextend_` filename prefix is the contract — downstream audits
...
- **un-automatable** — keep as manual; do not emit an AFS; report to the caller
...
- Determine **where** the ticket lands by reading
  ...
  **Bug filing style** — the *shape* of the ticket. Three styles:
  ...
  - **`story-subtask`** — create a sub-task under the originating
    story (Jira / Azure DevOps only). Requires a parent story ID
    from the caller; pass it as the parent when handing off to the
    bug-filing skill.
...
- Determine **whether to bundle or split** by reading
  ...
  - **`bundle-per-case`** *(opt-in, requires umbrella-ticket
    ...
    - If the finding is a *lightweight clarification* → check if
      there's already an open "umbrella" ticket for this scenario.
      - If yes: add the finding as a comment on the existing ticket.
      - If no: file a new umbrella ticket (title e.g.
        "Clarifications for <scenario-slug>") and make this the first
        comment. Future lightweight findings on the same scenario
        attach here.
...
A single markdown file per scenario, per the structure in
[`references/spec-format.md`](references/spec-format.md). Path:

```
test-specs/<feature>/l<priority>_<slug>.md
```

If a TMS case ID was provided, append it: `l<priority>_<slug>_<tms-id>.md`.
...
```
test-results/screenshots/<slug>-step-<n>-<action>.png
test-results/json/<slug>-<iso-timestamp>.json
```
...
1. Commit the AFS on a feature branch — `test(spec): add AFS for <slug>`
...
- **Skipping Phase 0 (case-gate)** when a TMS case ID was provided.
  Status / folder-membership drift between batches — re-probe per
  case, every dispatch. (Phase 0 is skipped for feature files and
  plain text — that's by design, not an anti-pattern.)
